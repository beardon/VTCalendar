<?php
  ...
  if (!(isset($title) && isset($header) && isset($footer) && 
    ...
    $todaycolor = $_SESSION["TODAYCOLOR"];
	...
	if ( isset($save) ) {
		...
		if (empty($addPIDError)) { 
			...
			$result = DBQuery($database, "UPDATE vtcal_calendar SET title='".sqlescape($title)."',header='".sqlescape($header)."',footer='".sqlescape($footer)."',bgcolor='".sqlescape($bgcolor)."',maincolor='".sqlescape($maincolor)."',todaycolor='".sqlescape($todaycolor)."',viewauthrequired='".sqlescape($viewauthrequired)."',forwardeventdefault='".sqlescape($forwardeventdefault)."' WHERE id='".sqlescape($_SESSION["CALENDARID"])."'" ); 
	...
	pageheader("Change header, footer, colors, authorization - Event Calendar",
						 "Change header, footer, colors, authorization",
						 "Update","",$database);
  echo "<br>\n";
	box_begin("inputbox","Change header, footer, colors, authorization");
?>
...
function WebColorTable( colorVarName, colorImgName, initColor )
{
  ...
  <b>Calendar Title:</b> <font color="#999999">(empty or any text)</font><br>
  <input type="text" name="title" maxlength="<?php echo $constCalendarTitleMAXLENGTH; ?>" size="30" value="<?php 
	echo htmlentities($title);
	?>"><br>
  <br>

  <b>Header HTML:</b> <font color="#999999">(empty or any HTML)</font><br>
  <textarea name="header" wrap="physical" cols="70" rows="10"><?php 
	echo htmlentities($header);
	?></textarea><br>
  <br>

  <b>Footer HTML:</b> <font color="#999999">(empty or any HTML)</font><br>
...
<br>
    ...
    <b>Login required for viewing the calendar?</b>
</p>
...
<tr>
  <td align="right"><input type="radio" name="viewauthrequired" value="0"<?php 
	...
	?>></td>
  <td align="left">no login required; everyone can view the calendar<br></td>
</tr>
<tr>
  <td align="right" valign="top"><input type="radio" name="viewauthrequired" value="1"<?php 
	if ( $viewauthrequired != 0 ) { echo " checked"; }
	?>></td>
  <td align="left">login required; only the following user-IDs can view the calendar:<br>
<?php
  if (!empty($addPIDError)) {    
...
?>
  ...
  <br>  
  <br>
  <input type="submit" name="save" value="Save" class="button">&nbsp;&nbsp;<input type="submit" name="cancel" value="Cancel" class="button">
</form>
<?php
  box_end();
