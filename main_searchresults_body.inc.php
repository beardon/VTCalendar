...
?><table cellspacing="5" cellpadding="0" width="100%" bgcolor="#ffffff" border="0">
      <FORM method="post" action="main.php?view=search">
			<tr><td colspan="3">
			<br>
        <INPUT type="submit" name="back" value="&laquo; Back to previous page">
				<br><br>
			</td></tr>
<?php
			...
			if ( !empty($kw) ) {
			  // print featured text if exists
				if ( isset($search_featured) && array_key_exists ($kw, $search_featured) ) {
				  echo "<tr valign=\"top\">\n  <td colspan=\"3\">\n";
					echo '<table border="0" cellspacing="0" cellpadding="0" width="100%"><tr><td bgcolor="#ff9900">';
				...
				$query.= " and (";
				$query.="(e.title LIKE '%".sqlescape($kw)."%') or (e.description LIKE '%".sqlescape($kw)."%') or (e.displayedsponsor LIKE '%".sqlescape($kw)."%') or (c.name LIKE '%".sqlescape($kw)."%')";
        ...
        // check if there is a matching keyword in the database
				for ($j=0; $j<count($search_keyword); $j++) {
				  if ($kw==$search_keyword[$j]) {
	  			  ...
	  			  $query.=" or (e.title LIKE '%".sqlescape($kwalt)."%') or (e.description LIKE '%".sqlescape($kwalt)."%') or (e.displayedsponsor LIKE '%".sqlescape($kwalt)."%') or (c.name LIKE '%".sqlescape($kwalt)."%')";
					}
...
?>
        <tr valign="top">
          <td colspan="3"><br><span class="announcement">&nbsp;&nbsp;No events found.<br><br><br><br><br><br></span></td>
        </tr>
<?php	
    ...
    echo Month_to_Text_Abbreviation($event_timebegin['month'])," ",$event_timebegin['day'],", ",$event_timebegin['year'],"<br>";
    if ($event['wholedayevent']==0) {
      ...
      $previousWholeDay = true;
		...
		echo "          <td width=\"98%\"><a href=\"main.php?view=event&eventid=",$event['eventid'],"\"><b>",$event['title'],"</b></a> -\n";
