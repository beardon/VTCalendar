<?php
  ...
  if (isset($timebegin_month) && isset($timebegin_year)) {
    $timebegin=datetime2timestamp($timebegin_year,$timebegin_month,1,12,0,"am");
  }
  ...
  $showdate['timestamp_daybegin']=datetime2timestamp($showdate['year'],$showdate['month'],$showdate['day'],12,0,"am");
  $showdate['timestamp_dayend']  =datetime2timestamp($showdate['year'],$showdate['month'],$showdate['day'],11,59,"pm");
  ...
  // date of first Sunday in month1
  $month1['dow'] = Day_of_Week($month1['month'],1,$month1['year']);
  $month1start = Add_Delta_Days($month1['month'],1,$month1['year'],-$month1['dow']);
  $month1start['timestamp'] = datetime2timestamp($month1start['year'],$month1start['month'],$month1start['day'],12,0,"am");
  ...
  $month1lastday['timestamp'] = datetime2timestamp($month1lastday['year'],$month1lastday['month'],$month1lastday['day'],11,59,"pm");
	$month1end = Add_Delta_Days($month1lastday['month'],$month1lastday['day'],$month1lastday['year'],+6-$month1lastday['dow']);
  $month1end['timestamp'] = datetime2timestamp($month1end['year'],$month1end['month'],$month1end['day'],11,59,"pm");
  $month1['timestamp'] = datetime2timestamp($month1['year'],$month1['month'],$month1['day'],12,0,"am");

  // when does this particular week start and end?
  $dow = Day_of_Week($showdate['month'],$showdate['day'],$showdate['year']);
  $weekfrom = Add_Delta_Days($showdate['month'],$showdate['day'],$showdate['year'],-$dow);
  $weekto = Add_Delta_Days($showdate['month'],$showdate['day'],$showdate['year'],6-$dow);
  ...
  $weekfrom['timestamp']=datetime2timestamp($weekfrom['year'],$weekfrom['month'],$weekfrom['day'],12,0,"am");
  $weekto['timestamp']  =datetime2timestamp($weekto['year'],$weekto['month'],$weekto['day'],11,59,"pm");
  ...
  if ( $view == "day" ) { 
  	pageheader("Day - Calendar","","Day","",$database);
	}
	elseif ( $view == "week" ) { 
  	pageheader("Week - Calendar","","Week","",$database);
	}
	elseif ( $view == "month" ) { 
  	pageheader("Month - Calendar","","Month","",$database);
	}
	elseif ( $view == "event" ) { 
  	pageheader("Event - Calendar","","","",$database);
	}
	elseif ( $view == "search" ) { 
  	pageheader("Search - Calendar","","Search","",$database);
	}
	elseif ( $view == "searchresults" ) { 
  	pageheader("Search Results - Calendar","","Search","",$database);
	}
	elseif ( $view == "subscribe" ) { 
  	pageheader("Subscribe - Calendar","","Subscribe","",$database);
	}
	elseif ( $view == "filter" ) { 
  	pageheader("Filter - Calendar","","Filter","",$database);
	}
?>
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <td width="1%" rowspan="5" bgcolor="<?php echo $_SESSION["MAINCOLOR"]; ?>"><img src="images/spacer.gif" height="1" width="5" alt=""></td>
    <td width="1%" rowspan="3" bgcolor="<?php echo $bodycolor; ?>"><img src="images/spacer.gif" height="1" width="5" alt=""><br>
    </td>
...
<table width="100%"  border="0" cellspacing="0" cellpadding="2">
	<tr>
		<td bgcolor="#ff0000" style="color:#FFFFFF; font-size:0.7em">
		  &nbsp;<b>Showing only filtered events</b> (<?php 
...
require ( "main_".$view."_navpreviousnext.inc.php" );		  
    ...
    <td width="1%" rowspan="3" bgcolor="<?php echo $bodycolor; ?>"><img src="images/spacer.gif" height="1" width="5" alt=""></td>
    <td width="1%" rowspan="5" bgcolor="<?php echo $_SESSION["MAINCOLOR"]; ?>"><img src="images/spacer.gif" height="1" width="5" alt=""></td>
  </tr>
  <tr>
  ...
  if ( $view == "month" || !$enableViewMonth ) { 
    echo "<span style=\"color:#000000; font-weight:bold; text-decoration:none\">".substr(Month_to_Text($month1['month']),0,3)," ",$month1['year']."</span>";
  }
	else {
    echo "<a href=\"main.php?view=month&amp;timebegin=".urlencode(datetime2timestamp($month1['year'],$month1['month'],$month1['day'],12,0,"am"))."&sponsorid=".urlencode($sponsorid)."&categoryid=".urlencode($categoryid)."&keyword=".urlencode($keyword)."\" style=\"color:#0000ff; font-weight:bold; text-decoration:none\">";
    echo substr(Month_to_Text($month1['month']),0,3)," ",$month1['year'];
    echo "</a>";
	}					
?></td>
  ...
  for ($iweek=1; $iweek<=6; $iweek++) {
    ...
    if (($iweek < 5) || ($weekstart['month'] == $month1['month'])) {
      ...
      echo "<td class=\"littlecalendarday\" ";
			if ( $view == "week" ) { 
				if ( $weekstart['month']==$weekfrom['month'] && $weekstart['day']==$weekfrom['day'] && $weekstart['year']==$weekfrom['year'] ) {
  				echo "style=\"background-color:#cccccc\" ";
				}
			}
			echo "valign=\"top\" align=\"left\">\n";
      echo "<a style=\"text-decoration:none\" href=\"main.php?view=week&amp;timebegin=".urlencode($weekstart['timestamp'])."\">W",date("W",mktime(0,0,0,$weekstart['month'],$weekstart['day'],$weekstart['year']))+1,"</a></td>\n";
      ...
      for ($weekday = 0; $weekday <= 6; $weekday++) {
        ...
        echo "<td class=\"littlecalendarday\" ";
				if ( $iday['day']==$today['day'] && $iday['month']==$today['month'] && $iday['year']==$today['year']) {
  				echo "style=\"background-color:".$colortoday."\" ";
				} 
...
?>					
      ...
      <table width="100%" border="0" cellpadding="2" cellspacing="0" bgcolor="<?php echo $colortoday; ?>">
        <tr>
...
<?php
  ...
  if ( !($view=="day" && 
	...
	if ($showtodaylink) {
    echo "<a href=\"main.php?view=day&amp;timebegin=today\" style=\"color:#0000ff; text-decoration:none\">";	
	}
  echo "<b>";
	echo Day_of_Week_Abbreviation(Day_of_Week($today['month'],$today['day'],$today['year'])),", ";
	echo substr(Month_to_Text($today['month']),0,3)," ",date("j"),", ";
	echo $today['year'];
  echo "</b>";
	if ($showtodaylink) {
    echo "</a>";
...
<?php
  if ($view!='subscribe') {
		echo'    <a style="color:#0000ff; text-decoration:none" href="main.php?view=subscribe"><b>Subscribe &amp; Download</b></a>';
	}
	else {
	  echo '<b>Subscribe &amp; Download</b>';
	}
?>
			<br>
...
<?php
  if ($view!='filter') {
		echo'    <a style="color:#0000ff; text-decoration:none" href="main.php?view=filter"><b>Filter Events</b></a>';
	}
	else {
	  echo '<b>Filter Events</b>';
	}
?>
<br>
<br>
...
<tr>
  ...
  <td bgcolor="#eeeeee" align="right" valign="top">
   <span style="font-size:smaller">powered by <a style="text-decoration:none" href="http://opensource.isc.vt.edu/products/vtcalendar/">VT Calendar</a> 
	 <?php if (file_exists("VERSION")) { include('VERSION'); } ?></span>&nbsp;
  </td>
</tr>
</table> 
<?php
